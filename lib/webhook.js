...
WebHook.prototype.isValid = function(extraTokens) {
  ...
  if (!this.isBodyValid()) {
    console.log("IS BODY VALID FALSE");
    return false;
  }
  ...
  for (var i in tokens) {
    ...
    console.log("this.key == token.key",this.key == token.key);
    console.log('token.verify(this.body, this.signature)', token.verify(this.body, this.signature));
...
WebHook.prototype.isContentTypeValid = function() {
  console.log("isContentTypeValid", this.contentType === "application/json" || this.contentType    );
...
WebHook.prototype.isBodyValid = function() {
  console.log("isBodyValid",  this.data !== undefined || this.data);
